...
export function createSyntheticGenomeAudio(genome = {}, { durationSeconds = 2.5, sampleRate = DEFAULT_SAMPLE_RATE } = {}) {
  ...
  for (let i = 0; i < frames; i++) {
    ...
    const body = Math.sin(Math.PI * 2 * base * t) * 0.58
      + Math.sin(Math.PI * 2 * base * 1.5 * t) * harmonicity * 0.24
      + Math.sin(Math.PI * 2 * base * (2 + harmonicity) * t) * brightness * 0.12;
...
export function buildGeneticRenderProof({ sourceId = 'source', parentGenome, variant, familyId = null } = {}) {
  ...
  return {
    version: 'macsense-genetic-audio-render-proof-v1', sourceId, familyId,
    variantId: variant.variantId, action: variant.action, renderedAudio: true,
    sampleRate: variant.sampleRate, durationSeconds: variant.durationSeconds,
    parentTraits: variant.parentTraits, childTraits: variant.childTraits,
    deltas: variant.deltas, metrics: variant.metrics, exportedAt: new Date().toISOString()
  };
}
...
export function encodeWavBytes(audio) {
  ...
  writeString(view, 12, 'fmt '); view.setUint32(16, 16, true); view.setUint16(20, 1, true); view.setUint16(22, channels.length, true);
...
export function playRenderedAudio(audio, context) {
  ...
  source.buffer = audioToAudioBuffer(audio, context);
...
function processChannel(channel, deltas, childGenome, channelIndex, sampleRate) {
  ...
  const side = channelIndex === 0 ? -1 : 1;
  for (let i = 0; i < channel.length; i++) {
    const dry = channel[i] || 0;
    smooth = smooth * lowKeep + dry * (1 - lowKeep);
    const edge = dry - previous;
    previous = dry;
    const tremolo = 1 + Math.sin((i / sampleRate) * Math.PI * 2 * (0.18 + width * 0.34)) * width * 0.035 * side;
    output[i] = softClip((smooth + edge * high * transientLift) * drive) * (1 + side * width * 0.08) * tremolo;
  }
  return output;
}

function compareAudio(parent, child) {
  const p = measureAudio(parent), c = measureAudio(child);
  return { parent: p, child: c, loudnessLift: round(c.rms - p.rms, 5), peakDelta: round(c.peak - p.peak, 5), renderedFrames: child.channels[0].length };
}

export function measureAudio(audio) {
  assertAudio(audio);
  let sumSq = 0, peak = 0, count = 0;
  for (const channel of audio.channels) for (const value of channel) { const sample = Number(value) || 0; sumSq += sample * sample; peak = Math.max(peak, Math.abs(sample)); count++; }
...
function traitDeltas(parent = {}, child = {}) { return Object.fromEntries(TRAITS.map(trait => [trait, round((Number(child[trait]) || 0) - (Number(parent[trait]) || 0), 4)])); }
function pickTraits(genome = {}) { return Object.fromEntries(TRAITS.map(trait => [trait, round(Number(genome[trait]) || 0, 4)])); }
function normalizeAudio(audio, targetPeak = 0.95) { const peak = measureAudio(audio).peak || 1; return peak <= targetPeak ? audio : { ...audio, channels: audio.channels.map(channel => Float32Array.from(channel, value => value * targetPeak / peak)) }; }
function assertAudio(audio) { if (!audio || !Array.isArray(audio.channels) || !audio.channels.length) throw new Error('audio channels are required'); const length = audio.channels[0]?.length || 0; if (!length) throw new Error('audio must contain samples'); if (!audio.channels.every(channel => channel.length === length)) throw new Error('all channels must have the same length'); }
function writeString(view, offset, text) { for (let i = 0; i < text.length; i++) view.setUint8(offset + i, text.charCodeAt(i)); }
function softClip(value) { return Math.tanh(value * 1.2) / Math.tanh(1.2); }
